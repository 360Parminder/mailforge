...
export async function createEmail(emailData) {
    ...
    if (emailData.messageId) {
        data.messageId = emailData.messageId;
    }

    if (emailData.recipient) {
        data.recipient = emailData.recipient;
    }
...
export async function getEmailById(idOrMessageId) {
    if (!idOrMessageId) return null;

    // Try finding by primary key id first
    let email = await prisma.email.findUnique({
        where: { id: idOrMessageId }
    }).catch(() => null);

    if (email) return email;

    // Try finding by messageId (with or without brackets)
    const cleanMsgId = idOrMessageId.replace(/^<|>$/g, '');
    email = await prisma.email.findFirst({
        where: {
            OR: [
                { messageId: idOrMessageId },
